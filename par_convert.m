    ...
    switch lower(modality)
	   ...
	   if isempty(strfind(Parameters.sliceorient,'2'))~=1
        ...
        case 'bold'
            ...
            % reorient y
            cmd=sprintf('for file in %s/%s*.hdr;do reorient $file $file y o > /dev/null;done',subdir,modality);
            unix(cmd);
            
            % reorient x
            cmd=sprintf('for file in %s/%s*.hdr;do reorient $file $file x o > /dev/null;done',subdir,modality);
            unix(cmd);
            ...
            if strcmp(format,'afni')
                ...
                cmd=sprintf('to3d -time:zt %d %d %d alt+z -orient LPI -session . -prefix %s %s*.hdr 2>&1 | tee -a %s/afni_convert.log', ...
